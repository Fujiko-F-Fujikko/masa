...
class BoundingBoxEditor(QObject):  
    ...
    def normalize_bbox_coords(self, x1: int, y1: int, x2: int, y2: int) -> BoundingBox:  
        ...
        """  
        if not self.parent_widget or not self.parent_widget.video_manager:  
            # エラーハンドリングまたはデフォルト値の提供  
            return BoundingBox(0.0, 0.0, 0.0, 0.0)  
  
        # VideoPreviewWidgetの現在の表示サイズを取得  
        widget_width = self.parent_widget.width()  
        widget_height = self.parent_widget.height()  
  
        # オリジナルフレームのサイズを取得  
        original_width = self.parent_widget.video_manager.get_video_width()  
        original_height = self.parent_widget.video_manager.get_video_height()  
  
        if widget_width == 0 or widget_height == 0 or original_width == 0 or original_height == 0:  
            # ゼロ除算を防ぐためのチェック  
            return BoundingBox(0.0, 0.0, 0.0, 0.0)  
  
        # 表示サイズとオリジナルサイズの比率を計算  
        scale_x = original_width / widget_width  
        scale_y = original_height / widget_height  
  
        # ピクセル座標をオリジナルフレームのサイズにスケール  
        ...
        # 0.0-1.0の範囲に正規化  
        norm_x1 = scaled_x1 / original_width  
        norm_y1 = scaled_y1 / original_height  
        norm_x2 = scaled_x2 / original_width  
        norm_y2 = scaled_y2 / original_height  
