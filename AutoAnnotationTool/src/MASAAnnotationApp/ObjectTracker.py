...
class ObjectTracker:  
    ...
    def track_objects(self, frame: np.ndarray, frame_id: int,  
        ...
        try:  
            ...
            # initial_annotationsをdet_bboxesとdet_labelsに変換  
            ...
            if self.config.unified_mode:  
                track_result = inference_masa(  
                    ...
                    det_bboxes=det_bboxes_from_initial, # ここでinitial_annotationsから変換したものを渡す  
                    det_labels=det_labels_from_initial, # ここでinitial_annotationsから変換したものを渡す  
                ...
                track_result = inference_masa(  
                    ...
                    video_len=1000,  
                    test_pipeline=self.masa_test_pipeline,  
                    det_bboxes=det_bboxes, # 結合した検出結果を渡す  
                    det_labels=det_labels, # 結合したラベルを渡す  
                    fp16=self.config.fp16,  
                    show_fps=False  
                )  
